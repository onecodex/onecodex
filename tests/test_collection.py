...
def test_to_classification_df_no_taxa_exception(samples):
    results, tax_info = samples._collate_results(metric="readcount_w_children")
    species_tax_ids = [
        tax_id for tax_id in results.columns if samples.taxonomy.loc[tax_id, "rank"] == "species"
    ]
    results_without_species = results.drop(columns=species_tax_ids)

    with mock.patch.object(type(samples), "_collate_results") as mock_results:
