...
def upload_multipart(args, f):
    ...
    try:
        ...
        p = subprocess.Popen("aws s3 cp %s %s --quiet" % (f, s3_path), 
    ...
    if p.returncode != 0:
        print("An error occured uploading %s using the aws-cli." % f)
        print("Please ensure you have configured the aws-cli by running: \n"
               " aws configure \n")
        p.sigterm()
        sys.exit(1)
