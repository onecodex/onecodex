...
class StatsMixin:
    ...
    def _drop_all_nan_classifications(self, df: pd.DataFrame) -> pd.DataFrame:
        prev_count = len(df)
        df = df.drop([id_ for id_ in self._all_nan_classification_ids if id_ in df.index])
        num_dropped = prev_count - len(df)
        if num_dropped > 0:
            warnings.warn(
                f"{num_dropped} sample{'s were' if num_dropped > 1 else ' was'} excluded from the "
    ...
    def beta_diversity_stats(
        ...
        *,
        ...
        # Munge the metadata first in case there's any errors
        group_by = self._tuplize(group_by)
        df, magic_fields = self._metadata_fetch([group_by], coerce_missing_composite_fields=False)
        group_by_column_name = magic_fields[group_by]
        ...
        df = self._drop_all_nan_classifications(df)
