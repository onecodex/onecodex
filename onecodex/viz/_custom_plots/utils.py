...
def format_classification_results(results: dict) -> dict[str, Any]:
    """Format the ClassificationRun.results as a table."""
    ...
    for tax_id, data in results.get("data", {}).items():
        entry = {
            ...
            "filtered_readcount": data.get("x"),
            "filtered_readcount_w_children": data.get("y"),
        }
        ...
        if (
            ...
            and (entry["filtered_readcount"] is None or entry["filtered_readcount"] < 1)
            and (
                entry["filtered_readcount_w_children"] is None
                or entry["filtered_readcount_w_children"] < 1
            )
