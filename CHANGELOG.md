...
- Adds `match_taxonomy: bool` argument to plotting functions (`plot_*`) to
  disable matching taxa when specifying mappings between metadata to plot
  attributes (e.g., `tooltip`, `color`, `label`, `size`). The default is `True`
